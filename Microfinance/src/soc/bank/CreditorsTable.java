...
public class CreditorsTable {
	...
	public CreditorsTable(ResultSet rs, Statement stm) {
		this.rs = rs;
		this.stm = stm;
	}

	@SuppressWarnings("serial")
	public JPanel getCreditorsTable() {
		JPanel panel = new TranslucentJPanel(Color.BLUE);
		panel.setLayout(new BorderLayout());
		String query = "SELECT member,amount,month,date,time FROM bank_dep";
		double sum = 0;
		try {
			...
			if (rows > 0) {
				Object[][] data = new Object[rows][CreditorsHeader.header.length];
				String query1 = "SELECT member,amount,month,date,time FROM bank_dep";
				...
				DefaultTableModel model = new DefaultTableModel(data, CreditorsHeader.header);
				...
				TableRenderer.setJTableColumnsWidth(getTable(), 480, 20, 20, 20, 20, 20);
				...
				JLabel label = new JLabel("Total Deposit Amount $" + new DoubleForm().form(sum) + ".  "
