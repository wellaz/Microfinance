...
import soc.helpers.CreateDirectory;
...
public class BatchPDF {
	...
	public BatchPDF(JFrame frame, ResultSet rs, Statement stm) {
		this.frame = frame;
		this.rs = rs;
		this.stm = stm;
	}

	public void generatePDF(JTable jtable1, JTable jtable2, JTable jtable3, JTable jtable4, String name, String acc) {
		try {
			String filename = acc + ".pdf";
			...
			PdfPCell cell1 = new PdfPCell(new Paragraph("subscriptions  for".toUpperCase() + " " + name.toUpperCase(),
	...
	public class Worker extends SwingWorker<Void, Void> {
		...
		public void done() {
			...
			new AnimateDialog().fadeOut(dialog, 100);
			int done = JOptionPane.showConfirmDialog(frame,
					"All members' records have been downloaded\n and are all up to date.\n\nContinue to view the record?",
					"Success", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			if (done == JOptionPane.YES_OPTION) {
				String filename = "cashierdocs";
				String path = CommonPath.path() + filename;
				new PauseThread().pause(50);
				new OpenFile().open(path);
				String file = path + File.separatorChar + "batch.pdf";
				new PauseThread().pause(50);
	...
	public void concat() {
		try {
			new CreateDirectory().createDir("cashierdocs");
			Document d = new Document(PageSize.A4, 40, 40, 40, 40);
			PdfCopy writer = new PdfCopy(d, new FileOutputStream(
					CommonPath.path() + File.separatorChar + "cashierdocs" + File.separatorChar + "batch.pdf"));
			Rectangle rect = new Rectangle(30, 30, 550, 800);
			writer.setBoxSize("art", rect);
			d.open();
			...
			for (int id : allmembers) {
				reader = new PdfReader(CommonPath.path() + id + ".pdf");
	...
	public class MinireportsWorker extends SwingWorker<Void, Void> {
		JDialog dialog;
		JProgressBar prog;
		JButton hider;
		JLabel waitlbl;
		...
		public MinireportsWorker(JTable jtable1, JTable jtable2, JTable jtable3, JTable jtable4, String name,
				String acc) {
			...
			waitlbl = new JLabel("Pulling records....");
			dialog.addComponentListener(new ComponentAdapter() {
				@Override
				public void componentResized(ComponentEvent evvt) {
					dialog.setShape(new RoundRectangle2D.Double(0, 0, dialog.getWidth(), dialog.getHeight(), 5, 5));
		...
		protected Void doInBackground() throws Exception {
			...
			new AnimateDialog().fadeIn(dialog, 100);
			new AnimateDialog().fadeIn(dialog, 100);
			generatePDF(jtable1, jtable2, jtable3, jtable4, name, acc);
			return null;
		}
